...
% Adding paths
addpath('ct2mu')
addpath('positron_range')

% addpath('simulation')
% addpath('matlab')
% addpath('matlab\attenuation')
% addpath('matlab\ct2mu')
% addpath('matlab\positron_range')
% addpath('attenuation_sim')
% addpath('attenuation_sim\richSim')
% addpath('attenuation_sim\erikSim\sim2D')
% addpath('report_results')
% addpath('report_results\filesRich')
% addpath('report_results\filesErik')
% addpath('report_results\phantoms')
...
% load("....mat")
% toc
% save("file_path.mat", "X0mmSim", "X1mmSim", "X2mmSim", "X0rlmmSim", "ThetasSim",...
...
% save("file_path.mat.mat", "dataVecSampled", "X0mmSampled", ...
...
muVec = getAttenuationMEX(X1T, X2T,single(PETFOV),single(PETFOV),single(3.75),flipud(muCT),int32(size(muCTSim,1)),int32(size(muCTSim,2)),int32(numG)); % ONLY FLIPUD ON muCT SINCE WE X1,X2 IS CORRECTLY ORIENTED, ONLY X0 WAS FLIPPED FOR RECONSTRUCTION!!!!
toc
...
% % ROSS PLOTTING STUFF - MAKING SURE IT LOOKS CORRECT
% if numG<1E5
%     figure,
...
% UNCOMMENT TO LOAD INSTEAD OF COMPUTING EVERY TIME
% save("file_path.mat", 'AT1')
